...
- **開発計画時**: 「## 計画」- TDD開発フロー、従来開発フローとの使い分け
- **実装開始前**: 「## 設計」- アーキテクチャ設計、レイヤー分離、依存性注入設計、テスト戦略設計
- **コード実装時**: 「## 実装」- コード実装規則、品質基準、エラーハンドリング、ロギング
- **テスト実装時**: 「## テスト」- ユニットテスト設計、TDDサイクル実装、結合テスト設計、テスト実行フロー
- **デプロイ・運用時**: 「## 品質保証」- 開発完了基準、デプロイワークフロー、運用監視
